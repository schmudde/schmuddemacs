...
- icomplete: Standards compliant, feature parity with IDO, FIDO: a way to accept IDO quirks, Emacs 27 introduced many of these improvements
- Not standards compliant. Up to Emacs 26.3 it was fine.
    - IDO
    - Helm: Not standards compliant. filter-as-you-type completion

Standards compliant: when you define a prompt, you want it to be aligned with broader interface standards (e.g. you'll never need something like [[https://github.com/bbatsov/helm-projectile][helm projectile]] for Vertico).

~(savehist-mode 1)~ most used commands in the minibuffer prompt. Rather than [[https://github.com/nonsequitur/smex][smex]]. Smex is a package that improves the use of M-x. TODO: to use everywhere ~(global-set-key (kbd "M-x") 'smex)~

#+begin_src emacs-lisp
(savehist-mode 1)
#+end_src

*** VERTical Interactive COmpletion: vertico.el

- Vertico: purely a user-enhancement, based on the back end ("standard) completion framework
- Marginalia
- Orderless: hooks into the underlying completion infrastructure
...
~(global-corfu-mode 1)~ doesn't work in the terminal... check it out...

#+begin_src emacs-lisp
  (setq completion-in-region-function #'consult-completion-in-region)
#+end_src

*** COMMENT [[https://www.masteringemacs.org/article/introduction-to-ido-mode][Interactively DO Things]]: IDO
...
#+begin_src emacs-lisp :results none
  ...
  ;; fundamentally global-set-key, local-set-key, etc... call define-key
  ...
  (define-key my-keymap (kbd "c") 'quick-calc)
  (define-key global-map (kbd "C-z") my-keymap)
#+end_src
...
#+begin_src emacs-lisp
(global-set-key (kbd "C-z p") 'org-pomodoro)
#+end_src
