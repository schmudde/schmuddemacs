...
All about [[[https://www.djcbsoftware.nl/code/mu/mu4e/Contexts.html][contexts]]: It can be useful to switch between different sets of settings in mu4e; a typical example is the case where you have different e-mail accounts for private and work email, each with their own values for folders, e-mail addresses, mailservers and so on. Use ~;~ to switch contexts in mu4e.

Staying up-to-date with mu/mu4e:

- [[https://github.com/djcb/mu/releases][Release log]]
- [[https://github.com/emacsmirror/mu4e/blob/master/NEWS.org][NEWS (user visible changes & bigger non-visible ones)]]
...
MIME-part actions allow you to act upon MIME-parts in a message - such as attachments. For now, these actions are defined and documented in ~mu4e-view-mime-part-action~.
...
Note: the header listing the attachments is not updated but when you leave the email and reopen it, the attachment is gone
...
(defun my-remove-attachment (msg num)
  ...
  (let* ((attach (mu4e~view-get-attach msg num))
    ...
    (when (and filename
               (yes-or-no-p
                (format "Yo, with attachments I flow - you sure you want to remove '%s', bro?" filename)))
      (shell-command cmd)
      (message cmd))))
