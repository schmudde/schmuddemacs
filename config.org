...
lsp-mode ~use-package~ copied from Juan. He uses ~(setq lsp-clojure-custom-server-command ("bash" "-c" "clojure-lsp")~ to explore a Clojure codebase without having to start a REPL. He can essentially do anything besides evaluation. ~clojure-lsp~ uses ~clj-kondo~ under the hood to do the linting.
