...
#+begin_src emacs-lisp
  ...
  (setf completion-styles '(orderless basic flex)
...
There are many completion styles; builtin styles include ~flex~, which provides fuzzy completions.
...
How many research files (text and rich media) do I have? List and count the new lines.

#+begin_src shell
find /home/schmudde/Dropbox/notes -type f -print | wc -l
#+end_src
...
Patch for denote until I upgrade.
...
#+begin_src emacs-lisp
  ;; (add-to-list 'custom-theme-load-path "/home/schmudde/.emacs.d/themes/borland-blue-theme/")
...
I like to grep it grep it, I like to grep it grep it...

#+begin_src emacs-lisp
(grep-apply-setting 'grep-command "grep --color=auto -nHri --null -e")
#+end_src

I'm always opening windows from Emacs

#+begin_src emacs-lisp
(defun drs/xdg-open (path)
  "Prompt for PATH and open it with xdg-open."
  (interactive
   (list (read-file-name "xdg-open: " nil nil t)))
...
So I want to unbind it and then use it for something better. Fundamentally ~global-set-key~, ~local-set-key~, etc... call ~define-key~, so I'm going to use that for most of my keybindings.
...
#+begin_src emacs-lisp :results none
  ...
  (defvar my-keymap ; declare keymap and make some basic bindings
    (let ((map (make-sparse-keymap)))
      ...
      (define-key map (kbd "a") 'completion-at-point)
      (define-key map (kbd "o") 'drs/xdg-open)
      map)
...
#+name: /usr/local/share/emacs/28.3/lisp/org/org-faces.el.gz
#+begin_src emacs-lisp :tangle no
(defface drs-org-deadline
  '((((class color) (background light)) (:foreground "Purple" :underline f))
    (((class color) (background dark)) (:foreground "Cyan" :underline f))
    (f (:underline f)))
  "Face for Deadlines."
  :group 'org-faces)
#+end_src
...
Warning (treesit): The installed language grammar for javascript cannot be located or has problems (version-mismatch): 15
#+end_example

This is because Ubuntu dependencies are too old. So I tried some monkey business:
...
#+begin_src
  (use-package treesit-auto
    ...
    (setq treesit-auto-langs (seq-remove
                              (lambda (lang)
                                (memq lang '(c javascript)))
                              treesit-auto-langs))
...
#+begin_quote
[!TIP]
But then I had issues with markdown-ts-mode not existing in Emacs 30.1. But this could work itself out if I upgrade to Emacs 31.
#+end_quote

In the meantime, use only Clojure-TS Mode:
...
(use-package eca
  :config
  (keymap-unset eca-chat-mode-map "C-c C-l" t))
#+end_src
...
**** [[https://github.com/clojure-emacs/clojure-mode][clojure-mode]], clojure-ts-mode, and [[https://github.com/clojure-emacs/cider][cider]]                  :installed:
...
I [[https://github.com/emacs-lsp/lsp-mode/issues/4940][filed a bug]] with Emacs 30.1 + lsp-mode 9.0.0 on [2025-12-16 Tue]:

#+begin_quote
Please version bump the f.el requirement to [[https://github.com/rejeep/f.el/releases/tag/v0.21.0][v0.21.0]]. The current requirement for LSP 9.0.0 is f-0.20.0. This created a problem when installing on Emacs 30.1:

#+begin_src
Compiling file .../elpa/lsp-mode-9.0.0/lsp-actionscript.el at Tue Dec 16 11:25:01 2025 Entering directory ‘.../elpa/lsp-mode-9.0.0/’ lsp-actionscript.el:27:11:

Error: Eager macro-expansion failure: (error "Shortdoc f function ‘f-glob’: bad keyword ‘:noeval*’")
#+end_src

I updated f.el [[https://github.com/rejeep/f.el/pull/132/commits/5bd87c6eb4a8608bae5f0313d8c680aedb40dabd][to the following commit]]:

#+begin_src
(package-vc-install
 '(f :url "https://github.com/rejeep/f.el"
     :rev "5bd87c6eb4a8608bae5f0313d8c680aedb40dabd"))
#+end_src

to fix ~(f-shortdoc.el)~: changed keyword ~:noeval~ to ~:no-eval~. And it all worked.
#+end_quote
...
#+begin_src emacs-lisp
  (use-package lsp-mode
    ...
    :config
    ;; the top-level binding in lsp-mode.el may have already fired with the
    ;; default prefix; this runs strictly after load, so it always wins.
