...
#+begin_src emacs-lisp
  ...
  (defun my-markdown-insert-link ()
    (interactive)
    (if (bound-and-true-p org-stored-links)
...
#+begin_src emacs-lisp
  (defun test-mu4e-part-action (_)
    (message "Does this return a file? << %s >>"
             (mu4e-message-field-at-point :path)))

  (add-to-list 'mu4e-view-mime-part-actions
               '(:name "Test-the-thing" :handler test-mu4e-part-action :receives temp))
#+end_src
...
~ /usr/local/share/emacs/site-lisp/mu4e/mu4e-message.el

Get the information:

- ~(mu4e-message-field-at-point :path)~ → ~"/home/schmudde/.local/share/mail/personal/INBOX/cur/1697530521.82007_2.pop-os,U=5023:2,S"~
- ~(test-mu4e-part-action nil)~ ~"Does this return a file? << /home/schmudde/.local/share/mail/personal/INBOX/cur/1697530521.82007_2.pop-os,U=5023:2,S >>"~
- ~(mu4e--view-gather-mime-parts)~ → ~((2 #<buffer  *mm*-715641> ("application/octet-stream" (name . "toolbox--tb-2798-pdf_1433511060.pdf")) base64 nil ("attachment") nil nil nil))~
